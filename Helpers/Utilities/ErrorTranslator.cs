...
namespace DataAccess.Utilities
{
    public static class ErrorTranslator
    {
        public static string toTR(string error)
        {
            ...
            else if (error.Contains("Value cannot be null"))
            {
                return "Verinlerin doğruluğundan emin olun.";
            }
            else if (error.Contains("String or binary data would be truncated"))
            {
                return "Girilen veri çok uzun. 'Color' sütunundaki veri, izin verilen maksimum sınırı aşıyor.";
            }
            else
            {
